...
def parse_game(
    ...
    if game_info is None:
        print(f"Game {season}_{season_phase}_{game_id} is invalid")
        return None
    if not is_final_game(game_info):
        print(f"Game {season}_{season_phase}_{game_id} is unfinished")
        return None
    
    parsed_game_id = parse_game_id(game_info)
    if game_id != parsed_game_id:
        print(f"Game id {season}_{season_phase}_{game_id} does not match parsed game id {parsed_game_id}")
        return None
